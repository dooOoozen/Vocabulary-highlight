# 生词本（Line Vocabulary）— 网页英语单词高亮、翻译与背诵
...
扩展优先使用本地词典，查不到的单词和句子翻译才调用有道 API。需要时请配置密钥：

1. 前往 [有道智云](https://ai.youdao.com/) 注册并登录。
2. 创建应用，选择「文本翻译」服务，获取 **应用ID（appKey）** 和 **应用密钥（appSecret）**。
3. 打开生词本 →「设置」，填写并「保存」，再「测试翻译」验证。

> 密钥仅保存在本地 `chrome.storage.local`，不会上传到其它服务器。
